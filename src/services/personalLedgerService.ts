...
export async function getContacts(userId: string): Promise<PersonalContact[]> {
  try {
    ...
    return snap.docs.map((d) => ({ contactId: d.id, ...d.data() }) as PersonalContact);
  } catch (err) {
    logger.error('personal.getContacts', '讀取聯絡人失敗', err);
    throw new ZplitError('EXPENSE_SAVE_FAILED', '讀取個人聯絡人時發生錯誤', err);
  }
}
...
export async function getContact(
  ...
  try {
    const snap = await getDoc(doc(db, `personalLedger/${userId}/contacts/${contactId}`));
...
export async function createContact(
  ...
  logger.info('personal.createContact', '建立聯絡人成功', { userId, contactId, displayName });
...
export async function updateContact(
  userId: string,
  contactId: string,
  data: Partial<Pick<PersonalContact, 'displayName' | 'avatarUrl' | 'linkedUserId'>>
): Promise<void> {
  try {
    ...
    logger.info('personal.updateContact', '更新聯絡人成功', { userId, contactId });
  } catch (err) {
...
export async function deleteContact(
  ...
  try {
    ...
    logger.info('personal.deleteContact', '刪除聯絡人成功', { userId, contactId });
  } catch (err) {
...
export async function getPersonalExpenses(
  ...
  try {
    ...
    return snap.docs.map((d) => ({ expenseId: d.id, ...d.data() }) as PersonalExpense);
  } catch (err) {
...
export async function addPersonalExpense(
  ...
  try {
    const ref = doc(collection(db, `personalLedger/${userId}/contacts/${contactId}/expenses`));
    ...
    logger.info('personal.addExpense', '個人帳務新增成功', { userId, contactId, expenseId: ref.id });
    return ref.id;
  } catch (err) {
...
export async function deletePersonalExpense(
  ...
  try {
    await deleteDoc(
      doc(db, `personalLedger/${userId}/contacts/${contactId}/expenses/${expenseId}`)
    );
    logger.info('personal.deleteExpense', '個人帳務刪除成功', { userId, contactId, expenseId });
  } catch (err) {
...
export async function settleAllWithContact(
  ...
  try {
    ...
    logger.info('personal.settleAll', '個人帳款結清成功', { userId, contactId, netAmount });
  } catch (err) {
