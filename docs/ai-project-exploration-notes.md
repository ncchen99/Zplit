...
- **收款方式：銀行帳號／LINE Pay（`src/services/paymentService.ts`）**：群組文件（邀請預覽公開 get）與 `users/{uid}`（任何登入者可 get）都不能放帳號，所以存兩處：`users/{uid}/private/payment` 只有本人能讀寫（設定頁 `src/pages/settings/PaymentAccountSection.tsx`），`groups/{gid}/payees/{uid}` 是每個群組的副本、只有成員讀得到（結算分頁訂閱）。儲存時用一個 batch 寫進自己所有群組；新加入的群組或同步失敗的，由 `GroupDetailPage` 打開群組時呼叫 `syncMyPayee` 補寫（每次重讀 private 文件、不快取，避免另一台裝置改過後被舊值蓋回）。移除成員時 `removeGroupMember` 會刪掉他的副本（Rules 允許成員刪「已不在 memberUids」的人的副本），刪除帳號前會先清空。Rules 驗證：銀行代碼（3 碼）與帳號（8–16 碼純數字）一起有或一起沒有、`linePay` 是 bool，至少要有一種。銀行清單在 `src/lib/banks.ts`。
  - 結算分頁：收款人有收款方式的列可以點開轉帳資訊 modal（同 ConfirmModal 的 `modal-box`：標題「轉帳給 X」，有 LINE Pay 時標題下一行註明；帳號／金額各一列、用分隔線不包卡片，右邊複製鈕只複製純數字，成功才變勾勾；底下「關閉」與「已轉帳，結清」，後者直接結清、不再跳確認）。複製用 `src/utils/clipboard.ts`（Clipboard API 失敗時退回 execCommand，LINE 內建瀏覽器等環境會用到）。「自己要付」的列在結清左邊多一顆灰色圓形按鈕（有銀行是 Landmark、只有 LINE Pay 是 Wallet）；只要有一列有這顆按鈕，其他列留同寬空位，箭頭才對齊。自己有待收款又沒設定時，列表下方出現一塊可點的灰底區塊（同「詳細資訊」的 `bg-base-200 rounded-xl`）連到個人資料頁。payees 監聽失敗（例如 Rules 還沒部署）時 `payees` 維持 null，提示與按鈕都不會出現。
  - 個人資料頁的收款帳號跟暱稱同一層（fieldset；下方一行左邊是「可以收 LINE Pay」勾選、右邊是儲存狀態），不用區塊。銀行用 `ActionSheetSelect`；它的彈出卡片高度是 7.5 列，最後一列露一半提示可以往下捲（手機捲軸只在捲動時出現），打開時捲到目前選的項目。
  - 明細分頁的結清記錄：左邊的頭貼換成綠色勾勾圓（`bg-primary/15 text-primary`），金額用同一個綠色；右下角是「付款人 → 收款人」，兩顆頭貼都用 `AvatarGroup`，跟一般花費的分帳頭貼同大小、同位置。
